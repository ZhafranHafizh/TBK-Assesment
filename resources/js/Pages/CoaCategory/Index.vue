                ...
                <div class="flex flex-col lg:flex-row gap-6 items-start">
                            ...
                            <table class="w-full text-left text-sm text-[#e6edf3]">
                                <thead class="bg-[#21262d] text-[#8b949e] text-xs font-semibold uppercase border-b border-[#30363d]">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3 w-32">Tipe</th>
                                        <th scope="col" class="px-4 py-3 w-32 text-right">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-[#30363d]">
                                    <tr v-for="category in categories.data" :key="category.id" class="hover:bg-[#1f2937] transition-colors duration-150">
                                        <td class="px-4 py-3 font-medium">{{ category.name }}</td>
                                        <td class="px-4 py-3">
                                            <span v-if="category.type === 'income'" class="inline-flex items-center rounded-full bg-[#238636]/15 px-2 py-0.5 text-[10px] font-medium text-[#3fb950] border border-[#238636]/30">
                                                Income
                                            </span>
                                            <span v-else class="inline-flex items-center rounded-full bg-[#f85149]/15 px-2 py-0.5 text-[10px] font-medium text-[#ff7b72] border border-[#f85149]/30">
                                                Expense
                                            </span>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <Link v-if="category.is_editable_full" :href="route('coa-categories.edit', category.id)" class="inline-flex items-center rounded bg-[#21262d] px-2.5 py-1.5 text-xs font-medium text-[#c9d1d9] border border-[#30363d] hover:bg-[#30363d] hover:text-[#e6edf3] transition-colors">Edit</Link>
                                                ...
                                                <button @click="confirmDeletion(category.id)" class="inline-flex items-center rounded bg-[#21262d] px-2.5 py-1.5 text-xs font-medium text-[#f85149] border border-[#30363d] hover:bg-[#f85149] hover:text-white transition-colors">Hapus</button>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr v-if="categories.data.length === 0">
                                        <td colspan="3" class="px-4 py-8 text-center text-[#8b949e]">Belum ada data kategori COA.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div v-show="showCreateForm" class="lg:w-1/3 w-full transition-all duration-300 ease-in-out">
                        <div class="rounded-md border border-[#30363d] bg-[#161b22] p-5 shadow-sm sticky top-6">
                            <h3 class="text-lg font-medium text-[#e6edf3] mb-4 border-b border-[#30363d] pb-2">Tambah Kategori Baru</h3>
                            <form @submit.prevent="submit" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p v-if="form.errors.type" class="mt-1 text-xs text-[#ff7b72]">{{ form.errors.type }}</p>
                                </div>
